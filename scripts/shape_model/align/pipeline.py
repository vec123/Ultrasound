...
OUTPUT_DIR = os.path.join(ALIGN, "output")

SHAPE_MODEL_MAT = os.path.join(PROJECT_ROOT, "scripts", "shape_model", "mat_model", "BabyFaceModel.mat")
MESH_VTP_PATH = os.path.join(PROJECT_ROOT, "US_samples", "US_samples_hr", "1", "20_semanas", "VTK", "1-01.vtp")
LANDMARK_POSITIONS = os.path.join(PROJECT_ROOT, "US_samples", "US_samples_hr", "1", "20_semanas", "VTK", "1-01.json")
SPECTRAL_DIR = os.path.join(PROJECT_ROOT, "spectral_data")
surface = load_vtp_mesh(MESH_VTP_PATH)
surface_vertices = get_vtp_vertices(surface)
mean, eigenfunction, eigenvalues, model_landmarks= load_mat_ssm(
    SHAPE_MODEL_MAT,  normalize = False
    )
...
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "mean.vtp"))

vtp =create_polydata(init_shape.reshape(-1,3))
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "init_shape.vtp"))

vtp =create_polydata(surface_vertices)
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "surface.vtp"))
...
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "new_target_landmarks.vtp"))
...
save_matching_field(refined_vertices.reshape(-1, 3), aligned_surface.reshape(-1, 3), 
                    valid_matches, filename=os.path.join(OUTPUT_DIR, "matching_field.vtp"))
...
for gamma in gammas:
    ...
    save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, f"nicp_refined_vertices_gamma_{gamma}.vtp"))

shape_fit = deepcopy(shape)
coeffs = shape_fit.fit_to_matching(aligned_surface, valid_matches, learning_rate = 1, gamma=0, chi_clamp= None)
nicp_refined_vertices = shape_fit.reconstruct()
vtp = create_polydata(nicp_refined_vertices.reshape(-1,3))
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "nicp_refined_vertices_free.vtp"))

shape_fit = deepcopy(shape)
coeffs = shape_fit.fit_to_matching(aligned_surface, valid_matches, learning_rate = 1, gamma=0, chi_clamp= 0.99)
nicp_refined_vertices = shape_fit.reconstruct()
vtp = create_polydata(nicp_refined_vertices.reshape(-1,3))
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "nicp_refined_vertices_clamped.vtp"))
...
"""
L, M, spectral_evals, spectral_evecs = load_spectral_data(
    os.path.join(SPECTRAL_DIR, "spectral_data.npz"))
print("got operators")
...
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "n_region_verts.vtp"))
...
save_vtp_mesh(vtp, os.path.join(OUTPUT_DIR, "deformed_vertices.vtp"))